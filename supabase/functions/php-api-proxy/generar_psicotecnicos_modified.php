...
log_psico("Usuario $id_usuario (rol: $rol_usuario) generará psicotécnicos ".($es_publico ? "PÚBLICOS" : "PRIVADOS"));
...
try {
  ...
  log_psico("BD:$db_name | proc=$id_proceso tema_ctx='$tema' secc_ctx='$seccion' -> tema_db='$tema_db' secc_db='$seccion_db' | pedir=$num_preguntas | meta=".($hasMeta?'on':'off')." | publico=$es_publico");
} catch (Throwable $e) {
  ...
  while ($pendientes > 0) {
    ...
    foreach ($items as $it) {
      ...
      if ($hasMeta && $tiene_columna_publico) {
        ...
        // Insert CON meta SIN es_publico
        ...
        // Insert SIN meta CON es_publico
        ...
        // Insert SIN meta SIN es_publico
