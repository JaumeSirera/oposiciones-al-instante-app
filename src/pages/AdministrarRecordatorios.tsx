...
interface RecordatorioFlashcard {
  id: number;
  id_usuario: number;
  fecha_envio: string;
...
export default function AdministrarRecordatorios() {
  ...
  const [configsFlashcards, setConfigsFlashcards] = useState<ConfigFlashcard[]>([]);
  const [historialFlashcards, setHistorialFlashcards] = useState<RecordatorioFlashcard[]>([]);
  const [loadingFlashcards, setLoadingFlashcards] = useState(true);
  const [busquedaFlashcards, setBusquedaFlashcards] = useState("");
  ...
  const cargarRecordatoriosFlashcards = async () => {
    try {
      ...
      if (data.success) {
        setConfigsFlashcards(data.configs || []);
      ...
      console.error("Error:", error);
      toast.error("Error al cargar recordatorios de flashcards");
    } finally {
  ...
  const enviarRecordatorioFlashcardAhora = async (idUsuario: number) => {
    if (!confirm("¿Enviar recordatorio de flashcards ahora a este usuario?")) return;
    ...
    try {
      ...
      if (data.success) {
        toast.success("Recordatorio de flashcards enviado");
        cargarRecordatoriosFlashcards();
      } else {
  ...
  const configsFlashcardsFiltradas = configsFlashcards.filter((c) => {
    const busquedaLower = busquedaFlashcards.toLowerCase();
    return (
      (c.nombre_usuario?.toLowerCase() || "").includes(busquedaLower) ||
      (c.email_usuario?.toLowerCase() || "").includes(busquedaLower)
    );
  });
      ...
      <div className="container mx-auto py-8 px-4 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              ...
              {isSA && <Badge variant="secondary" className="ml-2">Modo SA - Todos los usuarios</Badge>}
            </p>
          </div>
          <Button onClick={() => activeTab === "planes" ? cargarRecordatorios() : cargarRecordatoriosFlashcards()} variant="outline" size="icon">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="planes" className="space-y-6">
            ...
            {/* Tabla de Recordatorios de Planes */}
            <Card>
              <CardHeader>
          ...
          <TabsContent value="flashcards" className="space-y-6">
                ...
                <div className="max-w-md">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      value={busquedaFlashcards}
                      onChange={(e) => setBusquedaFlashcards(e.target.value)}
                      className="pl-9"
                    />
                  </div>
                </div>
            ...
            {/* Tabla de Configuraciones de Flashcards */}
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <span>Configuraciones de Recordatorios ({configsFlashcardsFiltradas.length})</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                {loadingFlashcards ? (
                  <div className="text-center py-8">{t('reminders.loading')}</div>
                ) : configsFlashcardsFiltradas.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    No hay configuraciones de recordatorios de flashcards
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead>Mín. pendientes</TableHead>
                          <TableHead>Último envío</TableHead>
                          <TableHead className="text-right">Acciones</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {configsFlashcardsFiltradas.map((config) => (
                          <TableRow key={config.id_usuario}>
                            <TableCell className="font-medium">
                              {config.nombre_usuario || `Usuario ${config.id_usuario}`}
                            </TableCell>
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <Mail className="h-4 w-4 text-muted-foreground" />
                            ...
                            <TableCell className="capitalize">{config.frecuencia}</TableCell>
                            <TableCell>{config.hora_envio?.substring(0, 5) || "-"}</TableCell>
                            <TableCell>
                              <span className="text-sm text-muted-foreground">
                                {config.dias_semana?.split(",").map(d => {
                                  const dias = ["", "L", "M", "X", "J", "V", "S", "D"];
                                  return dias[parseInt(d)] || d;
                                }).join(", ") || "-"}
                              </span>
                            </TableCell>
                            <TableCell>{config.min_pendientes}</TableCell>
                            <TableCell>
                              {config.ultimo_envio ? (
                                <div className="flex items-center gap-2">
                                  <Calendar className="h-4 w-4 text-muted-foreground" />
                                  {format(new Date(config.ultimo_envio), "dd MMM yyyy HH:mm", { locale: es })}
                                </div>
                              ) : (
                            ...
                            <TableCell className="text-right">
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => enviarRecordatorioFlashcardAhora(config.id_usuario)}
            ...
            {/* Historial de envíos de Flashcards */}
            {historialFlashcards.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle>Historial de Envíos Recientes</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="overflow-x-auto">
                    <Table>
                      ...
                      <TableBody>
                        {historialFlashcards.map((item) => (
                          <TableRow key={item.id}>
                            ...
                            <TableCell>{item.nombre_usuario || `Usuario ${item.id_usuario}`}</TableCell>
